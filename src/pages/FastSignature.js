...
function Document() {
  ...
  async function onSubmitDocument(data, { reset }) {
    try {
      ...
      api
        ...
        .then((resultUser) => {
          if (
            resultUser.data === "strangeUser" &&
            $("#checkboxTypeEletronic").is(":checked")
          ) {
            $("#alert-strange").removeClass("hidden");
            $("#alert-strange2").removeClass("hidden");
            setTimeout(() => {
              $("#alert-strange").addClass("hidden");
              $("#alert-strange2").addClass("hidden");
            }, 5000);
          } else {
            if (types[1]) {
              setSigned((searches) =>
                      ...
                      .then((result) => {
                        if (array.length === 1) {
                          ...
                          if (count === 0) {
                            api
                              .post(
                                `/email/eletronic/signature/${array[0].email}`,
                                {
                                  ...
                                  pendingDesc:
                                    pendingDocument[0].descriptionDoc,
                                }
                              )
                              .then((resultpending) => {});
                            count++;
  ...
  $("#checkboxTypeAprovador").change(function () {
    if ($("#checkboxTypeEletronic").prop("checked")) {
      $("#checkboxTypeEletronic").prop("checked", false);
    }
    if ($(this).is(":checked")) {
  ...
  $("#checkboxTypeObservador").change(function () {
    ...
    if ($(this).is(":checked")) {
      $("#checkboxTypeEletronic").attr("checked", false);
      $(".eletronic-signature").addClass("hidden");
    }
  });
  ...
  return loading === true ? (
    ...
    <div className="main-container">
      ...
      <div className="row">
        <div class="alert alert-danger hidden" role="alert" id="alert-strange">
          Para a segurança da assinatura eletrônica usuários que são não
        ...
        <div className="container-boards col-lg-5">
          <div
            ...
            id="alert-strange2"
          >
            Assinatura eletrônica somente para usuários cadastrados no sistema
          </div>
