...
function DocumentStrangeSign() {
      ...
      axios.all([api.get(`/pending/${id}`)]).then(
        axios.spread((pendingInfo) => {
          if (pendingInfo.data !== null) {
            api.get(`/user/order/${id}`).then((findOrdens) => {
              setCurrentOrder(findOrdens.data);
            });
            api.get(`/organization/pendingPaste/${id}`).then((findOrg) => {
              setCurrentOrg(findOrg.data[0]);
            });
            api.get(`/user/paste/${id}`).then((findPaste) => {
              setCurrentPaste(findPaste.data);
            });
            api
              .get(`/user/strangeorder/${strangeId}/${id}`)
              .then((strangeOrder) => {
                setFindOrder(strangeOrder.data);
                setFindUser(strangeOrder.data);
              });

            setFindPending(pendingInfo.data);

            api.get(`/user/file/${pendingInfo.data.file}`).then((fileInfo) => {
              setCurrentFile(fileInfo.data);
            });
          }
            ...
            function start() {
              pki.listCertificates().success((certificates) => {
                ...
                function filterObj(obj) {
                  if (obj.pkiBrazil.cpf === findOrder.cpf) {
                    return obj;
                  }
                }
  ...
  async function startPades() {
    ...
    } else if (currentOrder[1].email === findUser.email) {
            ...
            <div className="row ">
              <div className=" card container-pdf col-lg-5">
                {findPending.action === 1 ? (
                  <div>
                    <h5>
                      disponível a apenas visualização do documento original
                    </h5>
              ...
              <div className=" card container-signature col-lg-5">
                ...
                <div className="document-container-information-list">
                  ...
                  <li>
                    {moment(findPending.createdAt).format("DD-MM-YY HH:mm:ss")}
                  </li>
                </div>
                {findOrder.type === "signatario" ? (
                  <div>
...
export default DocumentStrangeSign;
