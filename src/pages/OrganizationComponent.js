    ...
    axios
      ...
      .then(
        axios.spread((findInfo, loadpaste, organization) => {
          ...
          api
            .get(`/user/organizationDocuments/${newid}`)
            .then((findDocument) => {
              api.get(`/user/organization/${newid}`).then((result) => {
                api.get(`/pasteorganization/${newid}`).then((result) => {
                  setOrgPastes(result.data);
                });
                setOrg(result.data);

                setPending(findDocument.data.OrganizationPaste);
                setSigned(findDocument.data.OrganizationSigned);
          ...
          <li className="nav-item nav-paste">
            ...
            <p>
              <img
                ...
                alt=""
              />{" "}
              {org.nome} <small>{cnpjMask(org.number)}</small>
            </p>
            <a href={`/dashboard/organizationPastas/${id}`}>
              <img
                className="img-fluid img-backbutton"
                id="img-paste"
                title="pastas"
            ...
            <h5 title="pastas">({orgsPastes.length})</h5>
            {display && (
